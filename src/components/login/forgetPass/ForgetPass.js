  ...
  const resetPass = (event) => {
    event.preventDefault();
      if(event.target.username.value.trim()){
        ...
        .then((res) => {
            emailjs.send('service_i3g1obg', 'template_bl2hmw5', {name: res.data.user.username, email: res.data.user.email, appname: 'Home Manager', link:`http://localhost:3000/login/newPass/${res.data.token}`}, '5Ed-UnttEg7TCYwCB')
            .then((result) => {
                if(result.text === 'OK'){
